...
export function TicketDetailPage() {
  ...
  return (
    <PageShell>
      <TealBand variant="compact">
        <button onClick={() => navigate(-1)} style={{ background: 'none', border: 'none', color: 'rgba(251, 247, 240,0.7)', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: 'var(--text-label-mono)', fontWeight: 500, letterSpacing: 'var(--ls-label-mono)', textTransform: 'uppercase', marginBottom: 'var(--space-lg)', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      ...
      <Modal isOpen={showGiftModal} onClose={() => setShowGiftModal(false)} title="Gift Ticket"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-lg)' }}>
          ...
          <p className="type-body-xs" style={{ color: 'rgba(22, 16, 31,0.6)' }}>Gifting is permanent and cannot be reversed. The recipient must have a Festify account.</p>
        </div>
      </Modal>
